...
```
tests/
├── backend/          # Тести бекенду (unit, integration, e2e, experimental)
├── ml/               # Тести ML моделей (unit, metrics, experimental)
...
# Тільки експериментальні (для диплому)
...
- **Unit-тести**: Тести окремих функцій та методів
- **Integration-тести**: Тести API endpoints з реальною БД
- **E2E тести**: Повні сценарії користувача
- **ML тести**: Перевірка моделей, метрик, детермінізму
- **Експериментальні тести**: Граничні випадки, чутливість, робастність (для дипломної роботи)

Детальніше про структуру та типи тестів див. `TESTING_STRUCTURE.md`.
